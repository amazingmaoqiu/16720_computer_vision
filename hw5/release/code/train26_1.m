...
% train_data = train_data(5:5:7800,:);
% train_labels = train_labels(5:5:7800,:);
...
for k = 1:1:30
    train_data_set = train_data(1:30:size(train_data,1) - 30 + k,:);
    train_labels_set = train_labels(1:30:size(train_data,1) - 30 + k,:);
    for j = 1:num_epoch
        [W, b] = Train(W, b, train_data_set, train_labels_set, learning_rate);

        [train_acc, train_loss] = ComputeAccuracyAndLoss(W, b, train_data_set, train_labels_set);
