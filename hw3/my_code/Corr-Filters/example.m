...
g = ((X*X' + eye(N))^-1)*X*y;
g1 = reshape(g,size(dp1));
img_filter_1 = imfilter(img,g1);
imagesc(img_filter_1);
g = ((X*X')^-1)*X*y;
g0 = reshape(g,[29,45]);
figure(2);
img_filter_0 = imfilter(img,g0);
imshow(img_filter_0);

[x1,y1] = find(img_filter_1 == max(img_filter_1(:)));
xl = x1 - floor(size(g1,2)/2);
xr = x1 + floor(size(g1,2)/2);
yl = y1 - floor(size(g1,1)/2);
yr = y1 + floor(size(g1,1)/2);
figure(2);
imshow(img);
hold on;
plot([xl,xr,xr,xl,xl],[yl,yl,yr,yr,yl],'LineWidth',1);
